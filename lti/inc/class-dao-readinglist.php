...
require_once "lib/php-project-framework/functions.php";
...
*/
	...
	private function findAlmaReadingList($course) {
		...
		// add the citation bib data to each citation
		/*
		$len = count($citations);
		for ($i=0; $i < $len; $i++) {
			if ( isset($citations[$i]['metadata']['mms_id']) ) {
				$mms_id = $citations[$i]['metadata']['mms_id'];
				$temp = getAlmaData("/bibs/".$mms_id, "view=brief");
				logMsg("Bib Extracted");
				logMsg($temp);
				$citations[$i]['bib'] = $temp;			
			}
		}
		*/
	 ...
	 *  
	 */
	private function apiAlmaGetBib($mms_id) {

		return $this->getAlmaData("/bibs/".$mms_id, ["view" => "full", "expand" => "None"]);
	...
	private function apiAlmaFormatReadingList( $items = array() ) {
		...
		$readinglist = $reOrdered; // replace
		$reOrdered = array(); // clear it out
		...
		if ( array_key_exists($citation['section_info']['id'], $readinglist) ) {
			...
			if ($citationVisible) {
				...
				if ( isset($citation['metadata']) ) {
					...
					// sometimes the readinglist record doesn't have full info, so we can get it from the bib if there
					if ( isset($citation['metadata']['mms_id']) ) {
						$mms_id = $citation['metadata']['mms_id'];
						$bib = $this->apiAlmaGetBib($mms_id);
						if ( isset($bib['title']) ) {
							$title = $bib['title'];
						}
						if ( isset($bib['author']) ) {
					...
					// if we don't yet have a title, fill it in
					if ($title === "") {
				...
				if ( $title === "" ) {
					...
					$from = trim(rtrim(trim($from), '/')); // get rid of the final / found in some titles, and remove all extra spaces before and after slash removal
					$from = trim(rtrim(trim($from), '.')); // get rid of the final . found in some titles, and remove all extra spaces before and after dot removal
				}

				// this is where if no mms id it should use source url
				$linkType = $this->alma['link'];
				//$materialURL = $citation[$linkType];
