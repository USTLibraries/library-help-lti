...
require_once __DIR__."/lib/php-project-framework/functions.php";
...
*/
	...
	public function init() {
		...
		if( $this->display['databases'] || $this->display['librarian'] || $this->display['guides'] ) {
            ...
            if ( $this->override("subject_id") ) {
                ...
                $logStr = "DEPARTMENT";
                $subjID_str = $this->course['dept_subject_id'];
            }
            if ( $subjID_str !== "" ) { 
	...
	public function getLibAppCourseSearchString($course = array(), $index = 0 ) {
		...
		if (!isset( $course['dept']) || $course['dept'] === "XXXX") {
			...
			// To customize the ruleset create a copy into custom/rulesets/ruleset-libapps-[version].php where [version] is your own verison ID
...
*/
	 ...
	 */
	private function mapDeptToSubject($dept) {	
	...
	private function importMappingTable() {
        $file = __DIR__."/../custom/data/dept-subj-mapping.csv";
	...
	private function importLibGuideOverrides() {
        $data = array();
        $file = __DIR__."/../custom/data/override-libguides-search.json";
     ...
     *  from the LMS. This checks the custom/data/override-libguides-search.json file for 
