...
function getRuleSetLocation($ruleset = "libapps" ) {
	...
	if ( hasData( getCfg("rulesets")[$ruleset]) ) {
		$file = __DIR__."/../custom/rulesets/ruleset-".$ruleset."-".getCfg("rulesets")[$ruleset].".php";
	} else {
		$file = __DIR__."/ruleset-".$ruleset."-default.php";
	}
...
function generateProfileBoxExpanded($librarian) {
    ...
    $html .= "<div class='librarian-profile-epanded' id='profile-".$librarian['id']."-epanded'>\n";

    // add profile pieces
    if (isset($librarian['profile'])) {

        // profile widget (probably Appointment button) and link to profile page
        if(isset($librarian['profile']['widget_lc'])) {
            $html .= "<div class='librarian-profile-epanded-widget'>".$librarian['profile']['widget_lc']."</div>";
        }
        ...
        if ( isset($librarian['profile']['connect']) ) {
            ...
            // DISPLAY PHONE
            if ( isset( $librarian['profile']['connect']['phone'] ) ) {
                $phone = $librarian['profile']['connect']['phone'];
                if ( $phone ) {
                    $html .= "<a href='tel:".$phone."' data-category=\"Profile\">".$phone."</a><br />";
                }
            }

            // DISPLAY EMAIL
            $email = $librarian['email'];
            if ( isset($librarian['profile']['connect']['email']) ) {
                $email = $librarian['profile']['connect']['email'];
            }
            $html .= "<a href='mailto:".$email."' data-category=\"Profile\">".$email."</a><br />";

            // DISPLAY ADDRESS
            if ( isset($librarian['profile']['connect']['address']) && $librarian['profile']['connect']['address'] ) {
                $html .= nl2br($librarian['profile']['connect']['address']) . "<br />";
            }
